#!/usr/bin/env python3
"""Prueba la plantilla NATIVA de Qwen3.5 (USE_HERMES_TEMPLATE=0).

Hipotesis: la plantilla generica hermes-tool-use.jinja rompe la planificacion
multi-paso; la nativa del modelo la conserva (es lo que hace OrcaRouter).
"""
...
def run(inp, max_wait=1500):
    jid = requests.post(f"{B}/run", headers=H, json={"input": inp}, timeout=60).json()["id"]
    t0 = time.time()
    last = ""
    while time.time() - t0 < max_wait:
        ...
        if st == "COMPLETED":
            return s.get("output")
        if st == "FAILED":
            print("  FAILED:", json.dumps(s)[:600], flush=True)
            return None
        time.sleep(15)
...
def main():
    print("== diag (confirmar plantilla nativa) ==", flush=True)
    out = run({"diag": True})
    if out:
        d = out[0] if isinstance(out, list) else out
        d = d.get("diag") or d
        print("   phase        :", d.get("phase"))
        print("   tool_template:", d.get("tool_template"), "(False = NATIVA, lo que queremos)")
        print("   tpl head     :", str(d.get("server_template_head"))[:120])
        print("   tiene tool_call:", d.get("server_template_has_tool_call"))

    print("\n== encadenado de tools con plantilla NATIVA ==", flush=True)
    out = run({"messages": [{"role": "system", "content": SYS},
    ...
    print("=" * 64)
    if not tc:
        print("SIN tool_calls. Texto:", repr((msg.get("content") or "")[:400]))
        print("VEREDICTO: la plantilla nativa NO expone las tools -> descartada")
        return
    ...
    print(f"  VEREDICTO   : {'RESUELTO - ENCADENA COMO ORCAROUTER' if lee else 'sigue solo abriendo'}")
